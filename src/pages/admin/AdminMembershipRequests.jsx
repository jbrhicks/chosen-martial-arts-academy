...
import RequestDetail from "@/components/admin/membership/RequestDetail";
...
export default function AdminMembershipRequests() {
  ...
  const load = async () => {
    try {
      const all = await base44.entities.MembershipRequest.list("-created_date");
      setRequests(all);
    } catch (e) {}
    setLoading(false);
  };
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-px bg-[#A8A9AD]/20 border border-[#A8A9AD]/20">
        {[
          ...
          { label: "Contacted", value: requests.filter(r => r.status === "contacted").length, color: "text-blue-400" },
          { label: "Approved", value: requests.filter(r => r.status === "approved").length, color: "text-green-400" },
          { label: "Saved", value: requests.filter(r => r.status === "saved").length, color: "text-purple-400" },
        ].map(stat => (
      ...
      <div className="flex gap-1 border border-[#A8A9AD]/20 p-1 w-full sm:w-fit overflow-x-auto">
        {["pending", "contacted", "approved", "saved", "all"].map(t => (
          <button key={t} onClick={() => setFilter(t)} className={`flex-1 sm:flex-none px-4 py-2 text-sm font-medium tracking-wide capitalize whitespace-nowrap transition-colors ${filter === t ? "bg-[#C9A84C] text-black" : "text-[#A8A9AD] hover:text-white"}`}>
            {t}
          </button>
        ))}
      </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-20 text-[#A8A9AD]">No {filter} requests.</div>
      ) : (
        <div className="space-y-3">
          {filtered.map(r => (
            <button key={r.id} onClick={() => setSelected(r)} className="w-full flex items-center gap-4 border border-[#A8A9AD]/20 p-4 hover:border-[#C9A84C]/30 transition-colors text-left">
              <div className={`w-1.5 h-14 ${r.status === "pending" ? "bg-[#C9A84C]" : r.status === "contacted" ? "bg-blue-400" : r.status === "approved" ? "bg-green-400" : "bg-purple-400"}`} />
              ...
              <div className="text-right shrink-0">
                <p className="text-xs tracking-widest uppercase text-[#A8A9AD]">{r.status}</p>
      ...
      {selected && <RequestDetail request={selected} onClose={() => setSelected(null)} onProcessed={() => { setSelected(null); load(); }} />}
    </div>
  );
}