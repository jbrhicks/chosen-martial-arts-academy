...
import { Star, MessageCircle, Pin, Flag, Trash2, Send, Loader2, Share2, Megaphone, Trophy, BarChart3 } from "lucide-react";
...
export default function PostCard({ post, currentUser, onPostUpdated, onPostDeleted }) {
  ...
  useEffect(() => { if (post.like_count !== undefined) setLikeCount(post.like_count); }, [post.like_count]);
  useEffect(() => { if (post.share_count !== undefined) setShareCount(post.share_count); }, [post.share_count]);

  useEffect(() => {
    if (!currentUser) return;
    base44.entities.Like.filter({ post_id: post.id, user_id: currentUser.id }).then(likes => setLiked(likes.length > 0)).catch(() => {});
  }, [post.id, currentUser]);
  ...
  const handleComment = async (e) => {
    ...
    try {
      const created = await base44.entities.Comment.create({ post_id: post.id, author_id: currentUser.id, author_name: currentUser.full_name, content: newComment.trim() });
  ...
  const handleFlag = async () => { setActionLoading(true); try { await base44.entities.Post.update(post.id, { is_flagged: !post.is_flagged }); onPostUpdated?.(); } catch (e) {} setActionLoading(false); };
  ...
  return (
    <div className={`border p-6 transition-colors ${
      ...
      {/* Banners */}
      {isBroadcast && (
        <div className="flex items-center gap-2 text-xs tracking-widest uppercase text-[#A8A9AD] mb-3">
          <Megaphone size={12} /> Broadcast
        </div>
      )}
      {post.is_pinned && !isBroadcast && (
      ...
      {/* Media */}
      {post.media_url && post.media_type === "image" && <img src={post.media_url} alt="" className="w-full max-h-96 object-cover border border-[#A8A9AD]/20 mb-4" />}
      {post.media_url && post.media_type === "video" && <video src={post.media_url} controls className="w-full max-h-96 border border-[#A8A9AD]/20 mb-4" />}

      {/* Actions */}
      <div className="flex items-center gap-4 pt-4 border-t border-[#A8A9AD]/20">
        <button onClick={handleLike} className={`flex items-center gap-2 text-sm transition-colors ${liked ? "text-[#C9A84C]" : "text-[#A8A9AD] hover:text-[#C9A84C]"}`}>
          <Star size={18} className={liked ? "fill-[#C9A84C]" : ""} />
          {likeCount}
        </button>
        ...
        {(isAdmin || isAuthor) && (
          <div className="ml-auto flex items-center gap-2">
            {isAdmin && (
              <>
                <button onClick={handlePin} disabled={actionLoading} className={`p-1.5 transition-colors ${post.is_pinned ? "text-[#C9A84C]" : "text-[#A8A9AD] hover:text-[#C9A84C]"}`} title="Pin"><Pin size={16} /></button>
                <button onClick={handleFlag} disabled={actionLoading} className={`p-1.5 transition-colors ${post.is_flagged ? "text-red-400" : "text-[#A8A9AD] hover:text-red-400"}`} title="Flag"><Flag size={16} /></button>
              </>
            )}
      ...
      {showComments && (
        <div className="mt-4 pt-4 border-t border-[#A8A9AD]/20 space-y-3">
          ...
          <form onSubmit={handleComment} className="flex gap-2">
            <input type="text" value={newComment} onChange={e => setNewComment(e.target.value)} placeholder="Write a comment..." className="flex-1 bg-white/5 border border-[#A8A9AD]/20 px-4 py-2 text-sm text-white focus:border-[#C9A84C] focus:outline-none transition-colors" />
